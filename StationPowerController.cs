...
bool generatorsRunning = true;

public Program()
{
    gridId = Me.CubeGrid.ToString();

    GridTerminalSystem.GetBlocksOfType(hydrogenGenerators, block => block.IsFunctional && block.CustomName.Contains("HydrogenGenerator") && block.CubeGrid.ToString() == gridId);
    GridTerminalSystem.GetBlocksOfType(statusScreens, block => block.IsFunctional && block.CustomName.Contains("PowerStatus") && block.CubeGrid.ToString() == gridId);
    GridTerminalSystem.GetBlocksOfType(batteries, block => block.IsFunctional && block.CustomName.Contains("Battery") && block.CubeGrid.ToString() == gridId);
    GridTerminalSystem.GetBlocksOfType(oxygenTanks, block => block.IsFunctional && block.CustomName.Contains("Oxygen Tank") && block.CubeGrid.ToString() == gridId);
    GridTerminalSystem.GetBlocksOfType(hydrogenTanks, block => block.IsFunctional && block.CustomName.Contains("Hydrogen Tank") && block.CubeGrid.ToString() == gridId);

    hasOxygenTanks = oxygenTanks.Any();
    hasHydrogenTanks = hydrogenTanks.Any();

    if (!hydrogenGenerators.Any())
    {
        Log("No generators found!");
        return;
    }

    foreach (IMyFunctionalBlock generator in hydrogenGenerators)
    {
        generator.Enabled = true;
    }

    foreach (IMyFunctionalBlock screen in statusScreens)
    {
        screen.CustomData = "StationPowerController";
    }
...
public void Main(string argument, UpdateType updateSource)
{
    ...
    if (powerLevel < 50 && fuelLevel > 25 && !generatorsRunning) toggleGenerators(true);
    ...
    if (!statusScreens.Any())
    {
        textToShow = textToShow + "\n\tNo status screens found!\n";
    }

    textToShow = textToShow + "\n\t[" + getFillLevel(powerLevel) + "] " + String.Format("{0,4}", powerLevel) + "% | " + String.Format("{0,8}", "Power - " + powerLevel + "%");

    if (hasOxygenTanks) textToShow = textToShow + "\n\t[" + getFillLevel(oxygenLevel) + "] " + String.Format("{0,4}", oxygenLevel) + "% | " + String.Format("{0,8}", "Oxygen - " + oxygenLevel + "%");
    if (hasHydrogenTanks) textToShow = textToShow + "\n\t[" + getFillLevel(fuelLevel) + "] " + String.Format("{0,4}", fuelLevel) + "% | " + String.Format("{0,8}", "Fuel - " + fuelLevel + "%");
